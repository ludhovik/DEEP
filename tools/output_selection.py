...
SCALARS = {'C','T','Comp','Phase','P','Cnom0','Compnom0','C_phiavg','Comp_phiavg','T_nom0','T_phiavg'}
GRADIENTS = {f'grad_{axis}{scalar}{suffix}' for axis in ('r','theta','phi','s','z') for scalar in ('C','Comp') for suffix in ('','_full')}
FIELDS = VELOCITY | MAGNETIC | EMF | INDUCTION | SCALARS | GRADIENTS | {'N2','N2_full'}
...
class OutputSelection:
    def __init__(self, args):
        ...
        if getattr(args,'no_gradients',False) and set(self.names)&(GRADIENTS|{'N2','N2_full'}):
            raise ValueError('--output gradient/N2 fields conflict with --no-gradients.')
        if getattr(args,'no_m0_fields',False) and any('nom0' in v or 'phiavg' in v or v in GRADIENTS and not v.endswith('_full') or v=='N2' for v in self.names):
    ...
    def needs(self,name):
        ...
        if name in ('C','T'):
            return bool(names&{'C','T','Cnom0','C_phiavg','T_nom0','T_phiavg','N2','N2_full'}) or any(v.startswith('grad_') and v.endswith(('C','C_full')) for v in names)
        if name=='Comp':
            return bool(names&{'Comp','Compnom0','Comp_phiavg','N2','N2_full'}) or any(v.startswith('grad_') and v.endswith(('Comp','Comp_full')) for v in names)
...
def selected_native_fields(selection,raw,radii,theta,phi,parameters,args,*,n2_factors=None,source_format='',operations):
    ...
    def base(name):
        actual='C' if name=='T' and source_format=='magic_graph' else name
        if actual not in raw:raise ValueError(f'--output needs {name}, but this input does not contain it.')
        return raw[actual],radii[actual]
    ...
    def scalar_of(name):return 'Comp' if 'Comp' in name else ('T' if name.startswith('T') else 'C')
    for name in selection.names:
        if name in raw or name in ('ur','ut','up','Br','Bt','Bp','C','T','Comp','Phase','P'):a,r=base(name)
        elif name in ('Cnom0','Compnom0') or name.endswith(('_nom0','_phiavg')):
            scalar=scalar_of(name) if name in SCALARS else name.split('_')[0]
            a,r=base(scalar);a=op('mean' if name.endswith('_phiavg') else 'nom0',a)
            ...
            if not name.endswith('_full'):a=op('nom0',a)
        elif name in EMF:
        ...
        elif name in ('N2','N2_full'):
            def calculate_n2():
                available=[s for s in ('C','Comp') if s in raw]
                ...
                for scalar in available:
                    if n2_factors is not None:
                        ...
                        ek,ra,pr=(parameters[k] for k in ('Ek','RaT' if scalar=='C' else 'RaC','Pr' if scalar=='C' else 'Sc'))
